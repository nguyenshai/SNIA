...
class AntColonyOptimization(Optimizer):
    ...
    def solve(self, iterations=200):
        ...
        for _ in range(iterations):
            ...
            iter_best_cost = all_lengths[iter_best_idx]
            ...
            self.save_history(
                ...
                pheromone=pher.copy(),
                iter_best_tour=list(iter_best_tour),
                iter_best_cost=float(iter_best_cost),
                rho=rho,
                n_ants=n_ants,
                ant_mean=float(np.mean(all_lengths)),
                ant_min=float(np.min(all_lengths)),
                ant_max=float(np.max(all_lengths)),
                ant_std=float(np.std(all_lengths)),
            )
