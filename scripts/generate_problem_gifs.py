...
def gif_tsp(fps=6):
    ...
    def update(frame):
        if frame < n_edges:
            ...
            lines.append(ln)
            title_obj.set_text(f'Step {frame+1}/{n_edges}')
        elif frame == n_edges:
            res = prob.evaluate(route)
            feas = 'FEASIBLE' if res['feasible'] else 'INFEASIBLE'
            title_obj.set_text(f'Cost: {res["total_cost"]:.1f}  |  {feas}')
        return lines
...
def gif_knapsack(fps=3):
    ...
    def draw(frame):
        ax_bar.clear()
        ax_sc.clear()
        dark_2d(fig, ax_bar, title='')
        dark_2d(fig, ax_sc, title='')
