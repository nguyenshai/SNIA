...
def chart_cont_t90(data):
    ...
    prob_names = list(CONT_PROBLEM_REGISTRY.keys())
    fig, axes = plt.subplots(1, len(prob_names), figsize=(4 * len(prob_names), 6))
    if len(prob_names) == 1: axes = [axes]
    fig.patch.set_facecolor(BG)
...
def chart_disc_speed_bars(data):
    ...
    fig, axes = plt.subplots(1, len(prob_names), figsize=(5.5 * len(prob_names), 7))
    if len(prob_names) == 1:
        axes = [axes]
    fig.patch.set_facecolor(BG)

    for ax, pname in zip(axes, prob_names):
