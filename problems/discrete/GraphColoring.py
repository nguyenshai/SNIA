...
class GraphColoringProblem:
    ...
    def visualize(self, coloring=None, save_path=None):
        ...
        title = f'  |  {self.n_nodes} events, {self.n_colors} slots'
        if coloring:
        ...
        ax2.text(0.05, 0.95, ''.join(info), transform=ax2.transAxes, verticalalignment='top',
    ...
    def get_description(self) -> str:
        ...
        n_hard = sum(1 for e in self.edges if e.is_hard)
        n_soft = len(self.edges) - n_hard
        n_pre = sum(1 for ev in self.events if ev.pre_assigned_color is not None)
        return (
            '========================================================' + '\n'
            ...
            'OBJECTIVE\n'
            f'  Assign {self.n_nodes} events to {self.n_colors} time slots\n'
            ...
            f'  - Forbidden pairs: {len(self.forbidden_pairs)} pairs\n'
            ...
            f'  Search space: {self.n_colors}^{self.n_nodes} possibilities\n'
    ...
    def _spring_layout(n_nodes, edge_set, rng, iterations=50):
        """Force-directed layout — vectorized repulsion/attraction."""
        pos = np.array([(float(rng.uniform(-5, 5)), float(rng.uniform(-5, 5)))
                        for _ in range(n_nodes)])   # (n, 2)
        edge_list = np.array(list(edge_set), dtype=int) if edge_set else np.empty((0, 2), dtype=int)
        k = 2.0
        k2 = k * k
        ...
        for _ in range(iterations):
            ...
            np.fill_diagonal(dist_sq, 1e6)                          # ignore self
            dist = np.sqrt(dist_sq)
            force_mag = k2 / dist                                   # (n, n)
            # disp[i] += sum_j (diff[i,j] / dist[i,j] * force_mag[i,j])
            disp += np.sum(diff / dist[:, :, np.newaxis] * force_mag[:, :, np.newaxis] * 0.1, axis=1)

            # Attraction: edges only
            if len(edge_list) > 0:
                u_idx = edge_list[:, 0]
                v_idx = edge_list[:, 1]
                ediff = pos[u_idx] - pos[v_idx]             # (E, 2)
                edist = np.sqrt(np.sum(ediff ** 2, axis=1) + 1e-6)[:, np.newaxis]
                f = ediff / edist * (edist / k) * 0.1
                np.add.at(disp, u_idx, -f)
                np.add.at(disp, v_idx,  f)

            # Clamp displacement
            mag = np.sqrt(np.sum(disp ** 2, axis=1, keepdims=True)) + 1e-6
            scale = np.minimum(mag, 1.0) / mag
            pos += disp * scale

        return {i: (float(pos[i, 0]), float(pos[i, 1])) for i in range(n_nodes)}
