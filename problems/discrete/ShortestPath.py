...
class ShortestPathProblem:
    ...
    _risk_grid: np.ndarray = field(default=None, init=False, repr=False)

    def __post_init__(self):
        """Precompute the risk grid for O(1) lookup during evaluate."""
        self._build_risk_grid()

    def _build_risk_grid(self):
        """Precompute per-cell risk from all hazard zones (vectorized)."""
        n = self.grid_size
        if not self.hazard_zones:
            self._risk_grid = np.zeros((n, n), dtype=float)
            return
        rows = np.arange(n, dtype=float)
        cols = np.arange(n, dtype=float)
        R, C = np.meshgrid(rows, cols, indexing='ij')  # (n, n)
        risk = np.zeros((n, n), dtype=float)
        for hz in self.hazard_zones:
            dist = np.sqrt((R - hz.center_row) ** 2 + (C - hz.center_col) ** 2)
            mask = dist <= hz.radius
            intensity = np.where(mask, 1.0 - dist / (hz.radius + 1e-6), 0.0)
    ...
    def cell_risk(self, r: int, c: int) -> float:
        """Total risk at cell (r,c) — O(1) lookup from precomputed grid."""
        if self._risk_grid is not None:
            return float(self._risk_grid[r, c])
    ...
    def evaluate(self, path: List[Tuple[int, int]]) -> Dict:
        ...
        total_risk = 0.0
        invalid_moves = []
        risk_grid = self._risk_grid  # local reference for speed
        ...
        for i in range(len(path) - 1):
            ...
            total_energy += cost
            total_risk += float(risk_grid[r2, c2]) if risk_grid is not None else self.cell_risk(r2, c2)
    ...
    def visualize(self, path=None, save_path=None):
        ...
        for hz in self.hazard_zones:
            ...
            ax.add_patch(circle)
            ax.annotate(f'RADrisk:{hz.risk_level:.1f}',
        ...
        for wp in self.waypoints:
            ...
            ax.annotate(f'{wp.name}+{wp.reward:.0f}',
        ...
        # Title
        title = f'  |  {self.grid_size}x{self.grid_size}'
        if path:
    ...
    def get_description(self) -> str:
        ...
        return (
            ...
            + (f'NP-hard (Steiner path)' if n_wp >= 2 else 'P (polynomial)' ) +
    ...
    def generate(cls, grid_size=30, difficulty='medium', seed=None):
        ...
        # Generate terrain types — vectorized
        elev = elevation
        terrain = np.where(
            elev > params['lava_thresh'], TERRAIN_LAVA,
            np.where(elev > 0.6, TERRAIN_ROCK,
            np.where(elev > 0.3, TERRAIN_SAND, TERRAIN_SMOOTH))
        ).astype(int)
        ...
        def find_passable(r_range, c_range):
            for _ in range(500):
                ...
                if terrain[r, c] != TERRAIN_LAVA:
                    return (r, c)
            # Exhaustive search fallback — scan the sub-grid
            for r in range(r_range[0], r_range[1]):
                for c in range(c_range[0], c_range[1]):
                    if terrain[r, c] != TERRAIN_LAVA:
                        return (r, c)
            # Last resort: force clear a cell
            fr, fc = r_range[0], c_range[0]
            terrain[fr, fc] = TERRAIN_SMOOTH
            return (fr, fc)
