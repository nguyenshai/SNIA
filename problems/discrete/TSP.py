...
class TSPProblem:
    ...
    def evaluate(self, route: List[int]) -> Dict:
        ...
        """
        # Validate: correct length AND correct city set (catches duplicates)
        if len(route) != self.n_cities or set(route) != set(range(self.n_cities)):
    ...
    def visualize(self, route: Optional[List[int]] = None,
        ...
        for storm in self.storm_zones:
            ...
            ax.add_patch(circle)
            ax.annotate(f'Storm+{storm.penalty:.0f}',
                        (storm.cx, storm.cy),
        ...
        for city in self.cities:
            ...
            # Time window label
            tw = city.time_window
            ax.annotate(f'{city.id}[{tw[0]:.0f}-{tw[1]:.0f}]',
    ...
    def get_description(self) -> str:
        ...
        n_depots = sum(1 for c in self.cities if c.is_depot)
        return (
            '=' * 56 + ''
            ...
            'OBJECTIVE'
            f'  Visit ALL {self.n_cities} cities once, return to origin,'
            ...
            f'  - Storm zones: {len(self.storm_zones)} areas on the map'
            f'    Crossing adds extra penalty to that edge'
            f'  - Fuel: max capacity = {self.fuel_capacity:.0f}'
            ...
            f'  Search space: ({self.n_cities}-1)!/2'
    ...
    def generate(cls, n_cities: int = 20, difficulty: str = 'medium',
        ...
        # --- Terrain multiplier ------------------------------------------
        # Vectorized generation: assign all edges at once
        terrain = np.ones((n_cities, n_cities))
        rand_vals = rng.random((n_cities, n_cities))
        # Mountain range: 1.5 + U(0, 0.8)
        mountain_extra = rng.random((n_cities, n_cities)) * 0.8
        mountain_mask = rand_vals < (0.15 + params['mountain_prob'])
        highway_mask = rand_vals < 0.15
        normal_mask = ~mountain_mask
        normal_noise = rng.random((n_cities, n_cities)) * 0.3
        terrain = np.where(highway_mask, 0.7,
                  np.where(mountain_mask, np.round(1.5 + mountain_extra, 2),
                           np.round(0.9 + normal_noise, 2)))
        # Make symmetric and zero diagonal
        terrain = np.triu(terrain, 1)
        terrain = terrain + terrain.T
        np.fill_diagonal(terrain, 0.0)
