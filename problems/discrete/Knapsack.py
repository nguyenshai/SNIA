...
class KnapsackProblem:
    ...
    def evaluate(self, selection: List[int]) -> Dict:
        """
        Evaluate a selection (binary list of length n_items, or list of selected indices).
        Returns dict with total_value, resource usage, violations, feasibility.
        """
        # Normalize: binary list vs index list
        # Use length as primary discriminator to avoid ambiguity when all
        # indices happen to be 0 or 1 (e.g. n_items=2).
        if len(selection) == self.n_items:
            sel_arr = np.asarray(selection)
            if sel_arr.dtype == bool or (np.all((sel_arr == 0) | (sel_arr == 1))):
                # Treat as binary mask
                selected_ids = set(np.where(sel_arr != 0)[0].tolist())
            else:
                selected_ids = set(int(s) for s in selection)
        else:
            selected_ids = set(int(s) for s in selection)

        if not selected_ids:
            return {
                'total_value': 0.0, 'base_value': 0.0, 'synergy_bonus': 0.0,
                'penalty': 0.0, 'n_selected': 0,
                'weight': {'used': 0.0, 'capacity': self.capacity_weight},
                'volume': {'used': 0.0, 'capacity': self.capacity_volume, 'effective': round(self.capacity_volume, 2)},
                'power':  {'used': 0.0, 'capacity': self.capacity_power},
                'active_synergies': [], 'violations': {}, 'feasible': True,
            }

        # Vectorized resource aggregation
        ids = list(selected_ids)
        total_weight = sum(self.items[i].weight for i in ids)
        total_volume = sum(self.items[i].volume for i in ids)
        total_power  = sum(self.items[i].power  for i in ids)
        base_value   = sum(self.items[i].value  for i in ids)

        # Synergy bonus — use a set for fast lookup
        synergy_bonus = 0.0
        active_synergies = []
    ...
    def get_description(self) -> str:
        ...
        n_crit = sum(1 for it in self.items if it.priority_class == 'critical')
        return (
            '=' * 56 + ''
            ...
            f'  - Weight capacity:  {self.capacity_weight:.0f}'
            f'  - Volume capacity:  {self.capacity_volume:.0f}'
            f'  - Power capacity:   {self.capacity_power:.0f}'
            f'  - Must include >= {self.min_critical_items} critical items'
            f'    (there are {n_crit} critical items available)'
            f'  - Conflicts: {len(self.conflicts)} forbidden item pairs'
            f'  - Synergies: {len(self.synergies)} bonus item pairs'
            'VALUE FORMULA'
            f'  Choose x in {{0,1}}^{self.n_items}  (select/skip each item)'
            ...
            f'  Search space: 2^{self.n_items} subsets'
