...
def launch_app(page: ft.Page):
    ...
    output = ft.TextField(
        ...
        read_only=True, value='', text_size=14, border_color='#30363d',
    )
    ...
    def collect_result(proc, rq):
        ...
        while True:
            try:
                payload = rq.get(timeout=0.2)
                break
            except Empty:
                if not proc.is_alive():
                    # Process died, flush one last time
                    try:
    ...
    def on_run(_):
        ...
        if action == 'visualize':
            ...
            desc = build_problem_description(job.get('category', ''), job.get('difficulty', 'easy'))
            desc_text.value = desc
            page.update()
        elif action == 'run':
            prob_lower = job.get('problem', '').lower()
            cat = 'tsp' if prob_lower == 'tsp' else 'knapsack' if prob_lower == 'knapsack' else 'graph_coloring' if 'color' in prob_lower else 'uninformed' if 'path' in prob_lower else 'all'
            desc = build_problem_description(cat, job.get('difficulty', 'easy'))
            if desc:
                desc_text.value = desc
                page.update()
